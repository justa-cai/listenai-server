...
客户端可在连接后通过 `set_audio_format` 命令切换音频格式。默认使用 PCM 格式。
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `command` | string | 是 | 固定为 `"set_audio_format"` |
| `format` | string | 是 | 音频格式：`"pcm"` 或 `"opus"` |
...
{
  "type": "error",
  "message": "Unsupported audio format: xxx. Supported: ['pcm', 'opus']",
  "code": 17,
  "timestamp": 1704067200000
}
```
...
**说明**:
- 可在连接后随时切换音频格式
- 切换格式后，后续发送的音频数据需使用新格式
- PCM: 客户端发送原始 PCM Int16 字节流
- OPUS: 客户端发送 OPUS 编码的音频包（需使用 OPUS 编码器）
- 默认格式为 PCM
...
async def asr_client_opus(url="ws://localhost:9200"):
    """使用 OPUS 格式的 ASR 客户端示例"""
    async with websockets.connect(url) as websocket:
        print("Connected to ASR server")
        ...
        print(f"Audio format set: {response}")

        # 初始化 OPUS 编码器
        sample_rate = 16000
        frame_size = 320  # 20ms @ 16kHz
        encoder = OpusEncoder(fs=sample_rate, channels=1, application="voip")
        encoder.bitrate = 24000  # 24 kbps

        # 模拟读取 PCM 音频数据
        # 这里假设 audio_data 是从麦克风或文件读取的 PCM Int16 数据
        # audio_data = b'\x00\x01\x02\x03...'  # 示例数据

        frame_duration_ms = 20
        frame_size_bytes = frame_size * 2  # Int16 = 2 bytes per sample

        # 分帧编码并发送
        async def send_audio():
            # 假设从某个源读取 PCM 数据
            while True:
                # 读取一帧 PCM 数据 (640 bytes = 20ms @ 16kHz Int16)
                # pcm_frame = read_audio_frame()  # 你的音频读取函数
                pcm_frame = b'\x00' * frame_size_bytes  # 示例：静音帧

                # 编码为 OPUS
                opus_packet = encoder.encode(pcm_frame, frame_size)
...
| **OPUS 音频格式配置** | | |
| `OPUS_ENABLED` | `true` | 是否启用 OPUS 格式支持 |
| `OPUS_SAMPLE_RATE` | `16000` | OPUS 采样率 (Hz) |
| `OPUS_CHANNELS` | `1` | OPUS 声道数 (1=单声道) |
| `OPUS_FRAME_SIZE` | `320` | OPUS 帧大小 (采样点数，20ms@16kHz) |
| `OPUS_FRAME_DURATION_MS` | `20` | OPUS 帧时长 (毫秒) |
| `OPUS_APPLICATION` | `"voip"` | OPUS 应用类型 (voip/audio/lowdelay) |
| `OPUS_BITRATE` | `24000` | OPUS 目标码率 (bps) |
| `DEFAULT_AUDIO_FORMAT` | `"pcm"` | 默认音频格式 (pcm/opus) |

**OPUS 音频格式说明**：
- OPUS 是一种低延迟、高压缩的音频编解码器
- 压缩比可达 10:1 ~ 12:1，相比 PCM 可节省约 90% 带宽
- 适合网络带宽受限的场景（如移动网络、远程传输）
- 客户端需要使用 OPUS 编码器对音频进行编码
- 推荐使用 `opuslib` (Python) 或 `OpusEncoder` (JavaScript) 进行编码
...
**OPUS 模式注意事项:**
1. 使用 OPUS 模式前，必须先发送 `set_audio_format` 命令将格式设置为 `"opus"`
2. OPUS 格式仅在客户端发送的二进制消息中生效，不影响 JSON 命令消息
3. OPUS 编码参数必须与服务端配置匹配：
   - 采样率: 16000 Hz
   - 声道: 单声道 (Mono)
   - 帧大小: 20ms (320 采样点)
   - 应用类型: voip
4. 每个发送的二进制消息应该是一个完整的 OPUS 编码帧
5. OPUS 包大小不固定，通常在 20-80 字节之间（取决于码率设置）
6. 切换回 PCM 模式：发送 `set_audio_format` 命令，`format` 设为 `"pcm"`
7. 建议发送间隔：20ms（与 OPUS 帧时长一致）
